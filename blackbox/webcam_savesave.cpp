...
//void thread_save_sigHandler(int sig);
//void thread_delete_sigHandler(int sig);

void * thr_ret;//thread qustn; 
...
//	void * thr_ret;
	...
	{
		puts("pthread_creat() error /*thread_save*/");
		return -1;
	};
       	if(pthread_join(delete_dir_id,&thr_ret)!=0)
	{
		puts("pthread_creat() error /*thread_delete*/");
		return -1;
	};
	free(thr_ret);
   	return 0;
...
void* thread_save(void *arg)
{
	...
	//signal(SIGINT,thread_save_sigHandler);
...
void* thread_delete(void* arg)
{
    ...
    //signal(SIGINT,thread_delete_sigHandler);
    MOUNTP *MP;
    if ((MP=dfopen()) == NULL)
    {
        perror("error");
...
void sigHandler(int sig)
{
	...
	printf(".....thread detach SUCCESS!! Maybe it causes some error message\n");
	sleep(10);
...
void thread_save_sigHandler(int sig)
{
	puts("thread_save got signal");

	pthread_exit((void *)2);	
}
void thread_delete_sigHandler(int sig)
{
	puts("thread_delete got signal");
	pthread_exit((void *)2);	
}
*/
